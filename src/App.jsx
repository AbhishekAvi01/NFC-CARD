...
//     setProfile({ ...editForm });
//     setIsEditing(false); 
//     setToastMessage("Workspace changes committed!");
//     setShowToast(true);
//     setTimeout(() => setShowToast(false), 3000);
//   };
...
//   return (
//     <div className="h-screen w-full bg-[#f8fafc] dark:bg-slate-950 flex flex-col overflow-hidden font-sans selection:bg-blue-600 selection:text-white transition-colors duration-300">
...
//       <div className={`flex-1 w-full overflow-y-auto ${layoutMode === 'mobile' ? 'px-0' : 'px-0'}`}>
//         <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 p-4 sm:p-6 lg:p-8">
          
//           {/* Profile Card (Sticky on Mobile & Desktop) */}
//           <div className={`${layoutMode === 'desktop' ? 'w-[400px] shrink-0' : 'w-full'} z-40`}>
//             <div className="sticky top-4 flex flex-col gap-4">
//               <LeftSidebar 
//                 profile={profile} visibility={visibility} isEditing={isEditing}
//                 editForm={editForm} handleInputChange={handleInputChange}
//                 saveProfileUpdates={saveProfileUpdates} cancelProfileUpdates={cancelProfileUpdates}
//                 setIsQrZoomed={setIsQrZoomed}
//               />
...
//           {/* Right Streams (Scrolls under the sticky card) */}
//           <div className="flex-1 flex flex-col gap-6">
//             {!isQrZoomed && (
//               <>
//                 <section className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200/60 dark:border-slate-800 shadow-sm">
//                   <ServicesGrid services={COMPONENT_DATA_PAYLOAD.services} />
//                 </section>
//                 <section className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200/60 dark:border-slate-800 shadow-sm">
//                   <MediaGallery galleryItems={COMPONENT_DATA_PAYLOAD.gallery} />
...
//       {/* QR Zoom Modal (Isolated Portal) */}
//       {isQrZoomed && createPortal(
//         <div className={theme === 'dark' ? 'dark' : ''}>
//           <div className="fixed inset-0 bg-[#1e2230] z-[999999] flex items-center justify-center p-4" onClick={() => setIsQrZoomed(false)}>
//             <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 max-w-xs w-full flex flex-col items-center gap-4 relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
//               <button onClick={() => setIsQrZoomed(false)} className="absolute top-4 right-4 text-slate-400 p-1 rounded-full bg-slate-100 dark:bg-slate-800"><X size={16}/></button>
//               <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(profile?.website || "https://github.com")}&ecc=M`} alt="QR" className="w-48 h-48" />
//               <span className="text-xs font-mono font-bold text-slate-500">{profile?.userId}</span>
//             </div>
//           </div>
//         </div>,
//         document.body
...
//       {showToast && (
//         <div className="fixed bottom-5 right-5 bg-slate-900 text-white text-xs font-bold px-4 py-3 rounded-xl z-[999999] flex items-center gap-2 border border-slate-800 animate-fade-in">
//           <CheckCircle2 size={14} className="text-emerald-400" />
//           <span>{toastMessage}</span>
//         </div>
//       )}
...
export default function App() {
  ...
  const [toastMessage, setToastMessage] = useState("");
  const [isQrZoomed, setIsQrZoomed] = useState(false);
  const [theme, setTheme] = useState(() => localStorage.getItem('app-theme-mode') || 'light');
  const [profile, setProfile] = useState(() => ({ ...(mockProfileData?.profile || {}), userId: mockProfileData?.profile?.userId || "@abhistark" }));
  const [visibility, setVisibility] = useState({ profilePicture: true, titleBlock: true, organizationBlock: true, bioBlock: true, contactStrip: true, qrSection: true });
  const [isEditing, setIsEditing] = useState(false);
  const [editForm, setEditForm] = useState({ ...profile });
  ...
  return (
    <div className="h-screen w-full bg-[#f8fafc] dark:bg-slate-950 flex flex-col overflow-hidden font-sans">
      <div className="w-full shrink-0 z-50">
        <Navbar 
          visibility={visibility} toggleVisibility={toggleVisibility}
          isEditing={isEditing} setIsEditing={(val) => { setEditForm({ ...profile }); setIsEditing(val); }}
          editForm={editForm} handleInputChange={handleInputChange}
          saveProfileUpdates={() => { setProfile({...editForm}); setIsEditing(false); }}
          cancelProfileUpdates={() => { setEditForm({...profile}); setIsEditing(false); }}
          currentAppTheme={theme} onChangeAppTheme={setTheme}
        />
      </div>
      ...
      <div className="flex-1 w-full overflow-y-auto">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 p-4 sm:p-6 lg:p-8">
          <div className={`${layoutMode === 'desktop' ? 'w-[400px] shrink-0' : 'w-full'} z-40`}>
            <div className="sticky top-0 pt-2 pb-4 bg-[#f8fafc] dark:bg-slate-950">
              <LeftSidebar profile={profile} visibility={visibility} isEditing={isEditing} editForm={editForm} handleInputChange={handleInputChange} setIsQrZoomed={setIsQrZoomed} />
            </div>
          </div>
          <div className="flex-1 flex flex-col gap-6">
            {!isQrZoomed && (
