...
// export default function LeftSidebar() {
//   // 1. Core Profile Data Pipeline State
//   const [profile, setProfile] = useState(mockProfileData.profile);
//   const [showControls, setShowControls] = useState(false);
//   const [showReviewModal, setShowReviewModal] = useState(false);
//   const [isQrZoomed, setIsQrZoomed] = useState(false); 

//   // 2. Visibility Controls Management Sub-State
//   const [visibility, setVisibility] = useState({
//     profilePicture: true,
//     titleBlock: true,
//     organizationBlock: true,
...
//   // 3. Edit Modes State Tracker Matrices
//   const [isEditing, setIsEditing] = useState(false);
//   const [editForm, setEditForm] = useState({ ...profile });
//   const [showToast, setShowToast] = useState(false);
//   const [toastMessage, setToastMessage] = useState("");

//   // 4. Live Review Submission Form Hooks State
//   const [reviewForm, setReviewForm] = useState({ rating: 5, comment: "", name: "" });

//   // Dynamic QR Pipeline logic tracking link updates
//   const activeTargetUrl = (isEditing ? editForm.website : profile.website) || `https://www.linkedin.com/in/abhishek-kumar-329a282b8/`; 
//   const liveQrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(activeTargetUrl)}&ecc=M`;
...
//       {/* 💳 Primary Upper Profile Block Card Module */}
//       <div className="w-full bg-[#030712] text-white rounded-[2rem] overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.2)] border border-slate-800/60 relative transition-all duration-300">
...
//         {/* 🛠️ WORKABLE DYNAMIC QR CONTAINER FRAME */}
//         {visibility.qrSection && (
//           <div className="absolute top-5 right-5 flex flex-col items-center gap-1 z-20 bg-slate-900/50 p-1.5 rounded-2xl border border-white/5 backdrop-blur-md group">
//             <div 
//               onClick={() => setIsQrZoomed(true)}
...
//             {/* 🛠️ 🟢 FIXED FULL SYNC: Input handles typing live updates on state perfectly */}
//             {isEditing ? (
//               <input 
//                 type="text" 
//                 name="userId" 
...
//             {/* 🛠️ 🟢 FIXED IN OVERLAY TOO: Maps target input states seamlessly */}
//             <span className="text-[10px] font-mono font-black text-slate-500 dark:text-slate-400 uppercase bg-slate-100 dark:bg-slate-950 px-2.5 py-1 rounded-xl tracking-widest border border-slate-200/50 dark:border-slate-800 transition-all">
//               {isEditing ? editForm.userId : profile.userId || "@abhistark"}
//             </span>
//           </div>
//         </div>
//       )}

//       {/* Interactive Customer Review Submission Dialog */}
//       {showReviewModal && (
//         <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
//           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-[2rem] p-5 shadow-2xl relative">
//             <button onClick={() => setShowReviewModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer"><X size={16} /></button>
...
//   setIsQrZoomed, 
//   setShowReviewModal, 
//   saveProfileUpdates,    // Main state save function straight from App.jsx
//   cancelProfileUpdates   // Main state cancel function straight from App.jsx
// }) {
...
//     if (item.action === "save_contact") {
//       e.preventDefault();
//       const event = new CustomEvent('navbarToastTriggerAction', { detail: { msg: "Contact Saved to Local Phone Stack!" } });
//       window.dispatchEvent(event);
//     }
//     if (item.action === "trigger_review") {
//       e.preventDefault();
//       if(setShowReviewModal) setShowReviewModal(true);
//     }
//   };
...
//       {/* 💳 Primary upper profile block card module workspace */}
...
//         {/* WORKABLE DYNAMIC QR CONTAINER FRAME */}
//         {visibility?.qrSection && (
...
//             {/* Inline dynamic live User ID handler */}
//             {isEditing ? (
...
//         {/* Dynamic Inner Text labels layout rendering pipeline */}
...
//               {/* 🛠️ 🟢 FIXED NAME FIELD VALUE BINDING */}
//               {isEditing ? (
//                 <input 
//                   type="text" 
//                   name="name" 
...
//               {/* 🛠️ 🟢 FIXED DESIGNATION VALUE BINDING */}
//               {visibility?.titleBlock && (
//                 isEditing ? (
//                   <input 
//                     type="text" 
...
//           {/* Dynamic Image URL and QR redirection links input box */}
//           {isEditing && (
...
//                 <input type="text" name="website" value={editForm?.website || ""} onChange={handleInputChange} className="bg-transparent text-emerald-400 text-[10px] font-mono w-full outline-none" placeholder="QR redirect domain path link" />
//               </div>
//             </div>
//           )}
...
//         {/* 🛠️ 🟢 FIXED CONTACT STRIP CHANNELS: Bound values directly to editForm variables */}
//         {visibility?.contactStrip && (
...
//                   onChange={handleInputChange} 
//                   className="bg-slate-950 border border-slate-800 text-white rounded px-1.5 py-0.5 text-[10px] outline-none font-bold w-28 focus:border-blue-500" 
//                 />
//               ) : (
//                 <a href={`tel:${profile?.phone}`} className="hover:text-blue-400 transition-colors break-all">{profile?.phone}</a>
//               )}
//             </div>
//             <div className="hidden md:block h-3.5 w-px bg-slate-800 shrink-0" />
...
//                   onChange={handleInputChange} 
//                   className="bg-slate-950 border border-slate-800 text-white rounded px-1.5 py-0.5 text-[10px] outline-none font-bold w-36 focus:border-blue-500" 
//                 />
//               ) : (
//                 <a href={`mailto:${profile?.email}`} className="hover:text-blue-400 transition-colors break-all">{profile?.email}</a>
//               )}
//             </div>
//             <div className="hidden md:block h-3.5 w-px bg-slate-800 shrink-0" />
//             <div className="flex items-center gap-2 shrink-0">
...
//                   onChange={handleInputChange} 
//                   className="bg-slate-950 border border-slate-800 text-white rounded px-1.5 py-0.5 text-[10px] outline-none font-bold w-24 focus:border-blue-500" 
//                 />
//               ) : (
//                 <span className="break-words text-slate-300">{profile?.location}</span>
//               )}
//             </div>
//           </div>
//         )}
...
//       {/* 🛠️ FLOATING GLASS ACTION COMMIT ACTION PANEL BUTTONS BAR ON CARD BOTTOM */}
//       {isEditing && (
//         <div className="w-full bg-blue-600/10 dark:bg-blue-500/5 backdrop-blur-md border border-blue-500/30 p-3 rounded-2xl flex items-center justify-between shadow-lg animate-scale-up z-30">
...
//             <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-tight">Perform direct changes on layout</span>
//           </div>
//           <div className="flex items-center gap-2">
//             <button 
//               type="button"
//               onClick={cancelProfileUpdates}
//               className="bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 p-2 rounded-xl transition-all active:scale-90 flex items-center gap-1 text-[11px] font-bold cursor-pointer"
//             >
...
//               className="bg-blue-600 hover:bg-blue-500 text-white p-2 rounded-xl transition-all active:scale-90 flex items-center gap-1 text-[11px] font-bold cursor-pointer shadow-md shadow-blue-600/20"
//             >
...
//                   rel={item.target ? "noopener noreferrer" : undefined}
//                   onClick={(e) => {
//                     if (item.action === "save_contact") { e.preventDefault(); handleActionClick(e, item); }
//                     if (item.action === "trigger_review") { e.preventDefault(); setShowReviewModal(true); }
//                   }}
//                   className="flex flex-col items-center justify-center py-3 px-1 bg-slate-50 dark:bg-slate-800/50 hover:bg-blue-50/50 dark:hover:bg-blue-950/30 active:scale-95 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xs hover:border-blue-100 dark:hover:border-blue-900 transition-all cursor-pointer text-center group"
//                 >
//                   <div className="text-slate-600 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{item.icon}</div>
...
//   editForm,
//   handleInputChange,
//   saveProfileUpdates,    // Handled perfectly from App thread
//   cancelProfileUpdates   // Handled perfectly from App thread
// }) {

//   // Defensive rendering path mappings
//   const activeTargetUrl = (isEditing ? editForm?.website : profile?.website) || `https://www.linkedin.com/in/abhishek-kumar-329a282b8/`; 
//   const liveQrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(activeTargetUrl)}&ecc=M`;
...
//     if (item.action === "save_contact") {
//       e.preventDefault();
//       const event = new CustomEvent('navbarToastTriggerAction', { detail: { msg: "Contact successfully queued into secure stack!" } });
//       window.dispatchEvent(event);
//     }
//     if (item.action === "trigger_review") {
//       e.preventDefault();
...
//       {/* 💳 Primary upper profile card module workspace */}
//       <div className={`w-full bg-[#030712] text-white rounded-[2rem] overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.3)] border relative transition-all duration-300 ${isEditing ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-800/60'}`}>
...
//               className={`w-12 h-12 bg-white rounded-lg p-0.5 flex items-center justify-center text-slate-950 shadow-md relative overflow-hidden ${isEditing ? 'cursor-not-allowed opacity-50' : 'cursor-zoom-in group/qr'}`}
//             >
//               <img src={liveQrApiUrl} alt="Matrix mapping Node Scanner" className="w-full h-full object-contain select-none" />
//               {!isEditing && (
//                 <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/qr:opacity-100 transition-opacity flex items-center justify-center text-white">
//                   <Maximize2 size={10} strokeWidth={3} />
//                 </div>
...
//             {/* Inline dynamic live User ID handler code fields */}
//             {isEditing ? (
//               <div className="flex items-center bg-slate-950/80 rounded border border-slate-800 px-0.5 w-14 overflow-hidden">
//                 <input 
//                   type="text" 
...
//         {/* Dynamic labels layout rendering pipeline Matrices */}
//         <div className="p-5 sm:p-6 flex flex-col gap-4 pt-16 pb-6 relative z-10 w-full">
//           <div className="flex flex-row items-start gap-4 sm:gap-6 w-full">
            
//             {/* Circular Avatar context block */}
//             {visibility?.profilePicture && (
//               <div className="relative shrink-0 transition-all duration-300">
//                 <div className="p-1 rounded-full bg-slate-900 border-2 border-emerald-500 shadow-md relative overflow-hidden">
//                   <img src={isEditing ? editForm?.imageUrl : profile?.imageUrl} alt={profile?.name || "User Avatar"} className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover bg-slate-800" />
//                 </div>
//                 <span className="absolute bottom-1 right-1 bg-blue-500 text-white p-1 rounded-full border border-slate-900 shadow-md flex items-center justify-center z-10"><ShieldCheck size={11} strokeWidth={3} /></span>
//               </div>
//             )}

//             {/* Labels Vs inputs Matrices Grid mapper logic */}
//             <div className="flex-1 min-w-0 flex flex-col items-start pt-1 w-full gap-1.5">
              
//               {/* 🛠️ 🟢 FIX REALTIME INPUT WIRES: Input values are temporary editForm states */}
//               {isEditing ? (
//                 <input 
//                   type="text" 
//                   name="name" 
//                   value={editForm?.name || ""} 
//                   onChange={handleInputChange} 
//                   className="w-full bg-slate-900/80 border border-slate-700/60 px-2 py-1 rounded-lg text-white font-bold text-sm outline-none focus:border-blue-500" 
//                   placeholder="Profile Name"
//                 />
//               ) : (
//                 <h2 className="text-base sm:text-lg font-black tracking-tight text-white flex items-center gap-1.5 w-full">
...
//               {/* Designation label text paragraph configuration mapper block */}
//               {visibility?.titleBlock && (
//                 isEditing ? (
//                   <input 
//                     type="text" 
...
//                     value={editForm?.title || ""} 
//                     onChange={handleInputChange} 
//                     className="w-full bg-slate-900/80 border border-slate-700/60 px-2 py-0.5 rounded-md text-blue-400 font-bold text-[11px] uppercase outline-none focus:border-blue-500" 
//                   />
//                 ) : (
//                   <p className="text-xs font-bold text-blue-400 tracking-wide uppercase">{profile?.title || "Developer"}</p>
//                 )
...
//               {/* Company organization paragraph data mapping coordinates */}
//               {visibility?.organizationBlock && (
//                 isEditing ? (
//                   <input 
//                     type="text" 
...
//                     value={editForm?.organization || ""} 
//                     onChange={handleInputChange} 
//                     className="w-full bg-slate-900/80 border border-slate-700/60 px-2 py-0.5 rounded-md text-slate-300 font-bold text-[10px] uppercase outline-none focus:border-blue-500" 
//                   />
//                 ) : (
//                   <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest block truncate w-full">{profile?.organization || "Organization"}</span>
//                 )
...
//           {/* Edit modes image or QR url domains blocks */}
//           {isEditing && (
//             <div className="flex flex-col gap-2 p-3 bg-slate-950 rounded-xl border border-slate-800/80 animate-fade-in text-left">
//               <div className="flex items-center gap-2 text-xs">
//                 <Camera size={12} className="text-slate-500" />
//                 <input type="text" name="imageUrl" value={editForm?.imageUrl || ""} onChange={handleInputChange} className="bg-transparent text-slate-300 text-[10px] font-mono w-full outline-none border-b border-slate-800 pb-0.5 focus:border-slate-700" placeholder="Avatar Image URL Link" />
//               </div>
//               <div className="flex items-center gap-2 text-xs">
//                 <Link size={12} className="text-slate-500" />
//                 <input type="text" name="website" value={editForm?.website || ""} onChange={handleInputChange} className="bg-transparent text-emerald-400 text-[10px] font-mono w-full outline-none focus:border-slate-700" placeholder="Domain Portfolio Link" />
//               </div>
//             </div>
//           )}

//           {/* Bio text block configuration node mapping coordinates */}
//           {visibility?.bioBlock && (
//             <div className="w-full">
//               {isEditing ? (
//                 <textarea 
...
//                   onChange={handleInputChange} 
//                   rows={2} 
//                   className="w-full bg-slate-900/80 border border-slate-700/60 p-2 rounded-lg text-[11px] text-slate-200 outline-none resize-none font-medium leading-tight focus:border-blue-500" 
...
//         {/* Interactive contact strip matrix row coordinate mappings */}
//         {visibility?.contactStrip && (
//           <div className="w-full bg-slate-900/60 border-t border-slate-800/80 px-4 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-2.5 text-[10px] md:text-xs font-bold text-slate-300 tracking-wide z-10 transition-all">
...
//             <div className="flex items-center gap-2 shrink-0">
//               <MapPin size={12} className="text-blue-500 shrink-0" />
//               {isEditing ? <input type="text" name="location" value={editForm?.location || ""} onChange={handleInputChange} className="bg-slate-950 border border-slate-800 text-white rounded px-1.5 py-0.5 text-[10px] outline-none font-bold w-24 focus:border-blue-500" /> : <span className="break-words text-slate-300">{profile?.location || "Delhi, India"}</span>}
//             </div>
//           </div>
//         )}
//       </div>

//       {/* 🛠️ 🟢 FIXED BAR VISIBILITY LOGIC: Bar listens to correct Save/Cancel thread */}
//       {isEditing && (
//         <div className="w-full bg-blue-600/10 dark:bg-blue-500/5 backdrop-blur-md border border-blue-500/30 p-3 rounded-2xl flex items-center justify-between shadow-lg animate-scale-up z-30">
//           <div className="flex flex-col text-left pl-1">
//             <span className="text-[10px] font-black uppercase text-blue-600 dark:text-blue-400 tracking-wider">Editing Workspace Active</span>
//             <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-tight">Perform direct changes on layout</span>
//           </div>
//           <div className="flex items-center gap-2">
//             <button 
//               type="button"
//               onClick={cancelProfileUpdates} // 🛠️ 🟢 FIXED THREAD: Discards bar instantly
//               className="bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 p-2 rounded-xl transition-all active:scale-90 flex items-center gap-1 text-[11px] font-bold cursor-pointer"
...
//             <button 
//               type="button"
//               onClick={saveProfileUpdates} // 🛠️ 🟢 FIXED THREAD: Hides bar after updating master state success
//               className="bg-blue-600 hover:bg-blue-500 text-white p-2 rounded-xl transition-all active:scale-90 flex items-center gap-1 text-[11px] font-bold cursor-pointer shadow-md shadow-blue-600/20"
...
//       {/* Quick Actions Grid Strip code blocks row */}
//       {!isEditing && (
//         <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-[2rem] border border-slate-200/60 dark:border-slate-800 shadow-xs relative w-full">
//           <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-3.5 px-1 text-left">Quick Actions</span>
//           <div className="grid grid-cols-3 gap-2">
...
//         <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center z-50 p-4 animate-fade-in">
//           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-6 max-w-xs w-full flex flex-col items-center gap-4 relative shadow-2xl">
//             <button onClick={() => setIsQrZoomed(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer p-1 rounded-full bg-slate-100 dark:bg-slate-800"><X size={16} /></button>
//             <div className="text-center">
//               <h4 className="text-xs font-black uppercase text-blue-600 dark:text-blue-400 tracking-widest">Scan Connection</h4>
//             </div>
//             <div className="w-48 h-48 bg-white p-3 rounded-2xl shadow-inner border border-slate-100 flex items-center justify-center">
//               <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(isEditing ? editForm?.website : profile?.website)}&ecc=M`} alt="Matrix Mapping Scanner" className="w-full h-full object-contain" />
//             </div>
//             <span className="text-[10px] font-mono font-black text-slate-500 dark:text-slate-400 uppercase bg-slate-100 dark:bg-slate-950 px-2.5 py-1 rounded-xl tracking-widest">{isEditing ? editForm?.userId : profile?.userId}</span>
//           </div>
//         </div>
//       )}
...
//         <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
//           <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-[2rem] p-5 shadow-2xl relative">
//             <button onClick={() => setShowReviewModal(false)} className="absolute top-4 right-4 text-slate-400 cursor-pointer"><X size={16} /></button>
//             <h3 className="text-xs font-black uppercase text-blue-600 dark:text-blue-400 tracking-widest mb-1">Submit Customer Review</h3>
//             <form onSubmit={(e) => {
//               e.preventDefault();
//               setShowReviewModal(false);
//               const event = new CustomEvent('navbarToastTriggerAction', { detail: { msg: "Thank you! Review submitted successfully." } });
//               window.dispatchEvent(event);
//               setReviewForm({ rating: 5, comment: "", name: "" });
//             }} className="flex flex-col gap-3.5 mt-4">
//               <div className="flex items-center gap-1">
//                 {[1, 2, 3, 4, 5].map((star) => (
//                   <button type="button" key={star} onClick={() => setReviewForm(prev => ({ ...prev, rating: star }))} className="cursor-pointer">
//                     <span className={`text-lg ${star <= reviewForm.rating ? "text-amber-400" : "text-slate-300"}`}>★</span>
//                   </button>
//                 ))}
//               </div>
//               <input type="text" required value={reviewForm.name} onChange={(e) => setReviewForm(prev => ({ ...prev, name: e.target.value }))} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs outline-none" placeholder="Your Name" />
//               <textarea required rows={3} value={reviewForm.comment} onChange={(e) => setReviewForm(prev => ({ ...prev, comment: e.target.value }))} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-xs resize-none outline-none" placeholder="Write feedback..." />
//               <button type="submit" className="w-full bg-blue-600 text-white font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer"><span>Submit Review</span></button>
//             </form>
//           </div>
//         </div>
//       )}
...
export default function LeftSidebar({
  ...
  setIsQrZoomed,
  setShowReviewModal,
  saveProfileUpdates,    // Main thread state updates handler
  cancelProfileUpdates   // Main thread state discard handler
}) {
    ...
    <div className="w-full flex flex-col gap-5 transition-all duration-300 animate-fade-in relative">
      ...
      <div className={`w-full bg-[#030712] text-white rounded-[2rem] overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.3)] border relative transition-all duration-300 ${isEditing ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-800/60'}`}>
        ...
        <div className="p-5 sm:p-6 flex flex-col gap-4 pt-16 pb-6 relative z-10 w-full">
          ...
          {/* Expanded URLs settings fields option list */}
          {isEditing && (
            <div className="flex flex-col gap-2 p-3 bg-slate-950 rounded-xl border border-slate-800/80 animate-fade-in text-left">
              <div className="flex items-center gap-2 text-xs">
                <Camera size={12} className="text-slate-500" />
        ...
        {/* Contact Strips info node parameters fields */}
        {visibility?.contactStrip && (
          <div className="w-full bg-slate-900/60 border-t border-slate-800/80 px-4 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-2.5 text-[10px] md:text-xs font-bold text-slate-300 tracking-wide z-10 transition-all">
            <div className="flex items-center gap-2 shrink-0 text-left">
              <Phone size={12} className="text-slate-500 shrink-0" />
      ...
      {/* 🛠️ 🟢 RESPONSIVE STICKY FIXED WORKSPACE MODE ACTIVE BAR */}
      {isEditing && (
...
// import { 
//   Phone, MessageSquare, Mail, Globe, MapPin, UserPlus, 
//   ShieldCheck, Camera, Maximize2, Wifi, MessageCircle, Check, X, Link 
// } from 'lucide-react';
...
//     if (item.action === "save_contact") {
//       e.preventDefault();
//       const event = new CustomEvent('navbarToastTriggerAction', { detail: { msg: "Contact saved to system!" } });
//       window.dispatchEvent(event);
//     }
//     if (item.action === "trigger_review") {
//       e.preventDefault();
...
//   return (
//     // 🛠️ REMOVED animate-fade-in to prevent sticky pinning jitter
//     <div className="w-full flex flex-col gap-5 transition-all duration-300 relative">
...
//               className={`w-12 h-12 bg-white rounded-lg p-0.5 flex items-center justify-center text-slate-950 shadow-md relative overflow-hidden ${isEditing ? 'cursor-not-allowed opacity-50' : 'cursor-zoom-in group/qr'}`}
//             >
//               <img src={liveQrApiUrl} alt="QR" className="w-full h-full object-contain select-none" />
//               {!isEditing && (
//                 <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/qr:opacity-100 transition-opacity flex items-center justify-center text-white">
//                   <Maximize2 size={10} strokeWidth={3} />
//                 </div>
...
//             {isEditing ? (
//               <div className="flex items-center bg-slate-950/80 rounded border border-slate-800 px-0.5 w-14 overflow-hidden">
//                 <input type="text" name="userId" value={editForm?.userId || ""} onChange={handleInputChange} className="bg-transparent text-blue-400 font-mono text-[8px] text-center w-full outline-none" placeholder="@id" />
//               </div>
//             ) : (
//               <span className="text-[8px] font-black text-slate-400 tracking-wide font-mono uppercase bg-slate-950/60 px-1 py-0.5 rounded transition-all">
//                 {profile?.userId || "@abhistark"}
...
//                   <img src={isEditing ? editForm?.imageUrl : profile?.imageUrl} alt="Profile" className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover bg-slate-800" />
//                 </div>
//                 <span className="absolute bottom-1 right-1 bg-blue-500 text-white p-1 rounded-full border border-slate-900 z-10"><ShieldCheck size={11} strokeWidth={3} /></span>
//               </div>
//             )}

//             <div className="flex-1 min-w-0 flex flex-col items-start pt-1 w-full gap-1.5">
//               {isEditing ? (
//                 <input type="text" name="name" value={editForm?.name || ""} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700 px-2 py-1 rounded-lg text-white font-bold text-sm outline-none" placeholder="Name" />
//               ) : (
//                 <h2 className="text-base sm:text-lg font-black tracking-tight text-white flex items-center gap-1.5 w-full truncate">{profile?.name} <span className="w-3.5 h-3.5 rounded-full bg-blue-500 flex items-center justify-center text-[8px] text-white">✓</span></h2>
//               )}
//               {visibility?.titleBlock && (isEditing ? <input type="text" name="title" value={editForm?.title || ""} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700 px-2 py-0.5 rounded text-blue-400 font-bold text-[11px] uppercase outline-none" /> : <p className="text-xs font-bold text-blue-400 uppercase">{profile?.title}</p>)}
//               {visibility?.organizationBlock && (isEditing ? <input type="text" name="organization" value={editForm?.organization || ""} onChange={handleInputChange} className="w-full bg-slate-900/80 border border-slate-700 px-2 py-0.5 rounded text-slate-300 font-bold text-[10px] uppercase outline-none" /> : <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{profile?.organization}</span>)}
//               <div className="w-10 h-0.5 bg-blue-500 rounded-full mt-1" />
//             </div>
//           </div>
...
//           {visibility?.bioBlock && (
//             <div className="w-full">
//               {isEditing ? <textarea name="bio" value={editForm?.bio || ""} onChange={handleInputChange} rows={2} className="w-full bg-slate-900/80 border border-slate-700 p-2 rounded-lg text-[11px] text-slate-200 outline-none resize-none" /> : <p className="text-[11px] text-slate-400 font-medium leading-relaxed">{profile?.bio}</p>}
//             </div>
//           )}
//         </div>

//         {visibility?.contactStrip && (
//           <div className="w-full bg-slate-900/60 border-t border-slate-800/80 px-4 py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-2.5 text-[10px] md:text-xs font-bold text-slate-300">
//             <div className="flex items-center gap-2 text-left"><Phone size={12} className="text-slate-500" /> {isEditing ? <input type="text" name="phone" value={editForm?.phone || ""} onChange={handleInputChange} className="bg-slate-950 border border-slate-800 w-28 px-1 outline-none" /> : <a href={`tel:${profile?.phone}`}>{profile?.phone}</a>}</div>
//             <div className="hidden md:block h-3 w-px bg-slate-800" />
//             <div className="flex items-center gap-2 text-left"><Mail size={12} className="text-slate-500" /> {isEditing ? <input type="text" name="email" value={editForm?.email || ""} onChange={handleInputChange} className="bg-slate-950 border border-slate-800 w-36 px-1 outline-none" /> : <a href={`mailto:${profile?.email}`}>{profile?.email}</a>}</div>
//           </div>
//         )}
//       </div>
...
//       {!isEditing && (
//         <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-[2rem] border border-slate-200/60 dark:border-slate-800 shadow-xs relative w-full">
//           <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-3.5 px-1 text-left">Quick Actions</span>
//           <div className="grid grid-cols-3 gap-2">
//             {actionItems.map((item, index) => (
//               <a key={index} href={item.href} onClick={(e) => handleActionClick(e, item)} className="flex flex-col items-center justify-center py-3 px-1 bg-slate-50 dark:bg-slate-800/50 hover:bg-blue-50/50 rounded-xl border border-slate-100 dark:border-slate-800 transition-all cursor-pointer group">
//                 <div className="text-slate-600 dark:text-slate-400 group-hover:text-blue-600">{item.icon}</div>
//                 <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 mt-1.5">{item.label}</span>
//               </a>
//             ))}
//           </div>
//         </div>
